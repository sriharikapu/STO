...
contract Whitelist is IWhitelist, Ownable {
    ...
    function isVerifiedAddress(address addr) internal view returns(byte _status){
        require(verified[addr] != ZERO_BYTES, "Address provided is not verified.");  
    }

    function isVerifiedSender(address addr) public view returns(byte) {
    ...
    function isVerifiedReceiver(address addr) public view returns(byte) {
        if(verified[addr] != ZERO_BYTES) {
            return hex"01";
        } else {
            return hex"57";
    ...
    function bulkWhitelist(address[] _addresses, bytes32[] _hashes) public onlyOwner {
       
        require(_addresses.length == _hashes.length, "Addresses and hashes should be of the same length.");
        
        uint i;
        for(i = 0; i < _addresses.length; i++) {
            addVerified(_addresses[i], _hashes[i]);
        }
...
contract SecurityToken is IERC20, ERC1410Standard, ERC1644, ERC1643, Pausable {
    ...
    constructor(string _name, string _symbol, address _owner, address _controller, address _whitelist_address) public ERC1644(_controller) {
        name = _name;
        symbol = _symbol;

        white_list = IWhitelist(_whitelist_address);
        transferOwnership(_owner);
    }
    ...
    function issueByPartition(bytes32 _partition, address _tokenHolder, uint256 _value, bytes _data) external onlyOwner {
        byte status = white_list.isVerifiedReceiver(_tokenHolder);
    ...
    function transferByPartition(bytes32 _partition, address _to, uint256 _value, bytes _data) external returns (bytes32) {
        ...
        byte status;
        status = white_list.isVerifiedReceiver(_to);
        require(status == hex"01", "Invalid receiver.");

        status = white_list.isVerifiedSender(msg.sender);
    ...
    function canTransferByPartition(address _from, address _to, bytes32 _partition, uint256 _value, bytes _data) external view returns (byte, bytes32, bytes32) {
        ...
        status = white_list.isVerifiedSender(_from);
        require(status == hex"01", "Invalid sender.");
